...
export function applyMove(state: GameState, tokenIdx: number, now: number = Date.now()): GameState {
...
export function recordRoll(state: GameState, dice: number, now: number = Date.now()): GameState {
  const s: GameState = JSON.parse(JSON.stringify(state));
  s.dice = dice;
