...
function Match({ game, setGame, onExit }: { game: GameState; setGame: (g: GameState) => void; onExit: () => void }) {
  ...
  const [myEquipped, setMyEquipped] = useState<Record<string, string>>({});
    ...
    const unsubAuth = auth.onAuthStateChanged((user) => {
      if (user) {
        unsubSnap = onSnapshot(doc(db, "profiles", user.uid), (snap) => {
          if (snap.exists() && snap.data().equipped) {
            setMyEquipped((prev) => ({ ...prev, ...snap.data().equipped }));
          }
        });
      } else if (unsubSnap) {
        unsubSnap();
  ...
  const doTokenMove = useCallback((seat: number, tokenIdx: number) => {
    if (isAnimating || seat !== displayGame.turn) return;
    const nextState = applyMove(game, tokenIdx);
    setGame(nextState);
  }, [game, displayGame.turn, isAnimating]);
