...
function RoomPage() {
  ...
  const getServerTime = useCallback(() => Date.now() + timeOffset, [timeOffset]);
  ...
  const rtdbGameRefCurrent = useRef<GameState | null>(null);

  usePresence(userId, code);

  useEffect(() => {
    roomRefCurrent.current = room;
    playersRefCurrent.current = players;
    rtdbGameRefCurrent.current = rtdbGame;
  }, [room, players, rtdbGame]);
  ...
  // RTDB listener for game state
  useEffect(() => {
    if ((room?.status !== "playing" && room?.status !== "finished") || !code) return;
    const stateRef = ref(rtdb, `rooms/${code}/state`);
    const unsub = onValue(stateRef, (snap) => {
      const val = snap.val();
      if (val) {
        setRtdbGame(val as GameState);
      }
    });
    return () => unsub();
  }, [room?.status, code]);
  ...
  async function onResign() {
    ...
    const next = resignPlayer(game, mySeat, getServerTime());
  ...
  async function pushState(newState: GameState, status?: string, additionalRoomUpdates?: any) {
    ...
    try {
      ...
      const result = await runRTDBTransaction(stateRef, (currentData) => {
        if (!currentData) return newState;
        
        // Prevent RACE CONDITIONS:
        // If the server's game state has progressed (turnStartTime changed) since we 
        // calculated this local move, we abort to avoid overwriting another player's move.
        if (currentData.turnStartTime !== game.turnStartTime) {
      ...
      // Update Firestore if there are structural room updates (like status changing to finished)
      if (status || additionalRoomUpdates) {
        const roomRef = doc(db, "rooms", code);
        const upd: any = { ...additionalRoomUpdates };
      ...
      if (e.message === "RACE_CONDITION") {
        console.warn("Race condition prevented: Server state already advanced.");
        return;
      }
  ...
  async function handleStateChange(next: GameState) {
    // Optimistic UI update: RTDB state first
    setRtdbGame(next);
  ...
  async function doRoll() {
    if (!game || mySeat !== game.turn || rolling || actionPendingRef.current) return;
    ...
    try {
      if (next.dice === null) {
        ...
        if (autoMoveIdx >= 0) {
          const finalState = applyMove(next, autoMoveIdx, getServerTime());
  ...
  async function doMove(_seat: number, tokenIdx: number) {
    if (!game || mySeat !== game.turn || actionPendingRef.current) return;
    actionPendingRef.current = true;
    setIsWaitingForServer(true);
    try {
  ...
  useEffect(() => {
    ...
    players.forEach((p) => {
      ...
      if (game.resigned?.includes(p.seat) || game.winners?.includes(p.seat)) return;
...
function ChatMenu({
  ...
  const myEquippedEmojiId = profiles[userId || ""]?.equipped?.emoji;
  const myCustomEmojis = STORE_ITEMS.find(i => i.id === myEquippedEmojiId)?.emojiTheme?.emojis || [];
  const ALL_MY_EMOJIS = [...myCustomEmojis, ...EMOJIS];
                ...
                {tab === "phrases" ? (
                  ...
                  <div className="flex flex-col gap-3">
                    ...
                    <button
                      onClick={() => {
                        if (customText.trim()) {
                          setSelectedContent({ type: "text", content: customText.trim() });
                        }
                      }}
                      className="p-3 bg-primary rounded-xl text-white font-bold disabled:opacity-50"
...
function OnlineMatch({
  ...
  useEffect(() => {
    if (isGameOver || !game?.turnStartTime) return;
    const interval = setInterval(() => {
      const elapsed = Math.floor((getServerTime() - game.turnStartTime) / 1000);
  ...
  async function sendReaction(targetSeat: number, emoji: string) {
    ...
    await updateDoc(doc(db, "rooms", code), {
      [`reactions.${targetSeat}`]: { emoji, sender: mySeat, timestamp: getServerTime() },
    });
  }
      ...
      {isGameOver && (
        <Podium 
          ...
          onAddFriend={async (friendId, friendName) => {
            ...
            try {
              await setDoc(doc(db, `profiles/${userId}/friends`, friendId), {
                id: friendId,
                addedAt: Date.now(),
              });
              await setDoc(doc(db, `profiles/${friendId}/friends`, userId), {
                id: userId,
                addedAt: Date.now(),
              });
              toast.success(`تمت إضافة ${friendName} إلى أصدقائك!`);
            } catch (e) {
          ...
          <div className="space-y-3">
            <div className="panel">
              ...
              <div className="mt-4 flex items-center justify-between">
                <Dice
                  value={displayGame.dice ?? displayGame.lastDiceRolled ?? null}
                  rolling={rolling}
                  size={72}
                  themeId={profiles[currentPlayer.userId || ""]?.equipped?.dice || "dice_default"}
                /><button onClick={doRoll} disabled={!canRoll} className="btn-game">
                  {rolling || isWaitingForServer
                    ? "..."
                    : myTurn
                      ? game.awaitingMove
                        ? "Choose token"
                    ...
                    {reactionTarget === i && (
                      <div className="absolute top-full right-0 z-50 mt-2 w-64 p-4 bg-black/90 backdrop-blur-xl rounded-2xl border border-white/20 shadow-2xl animate-in fade-in zoom-in-95">
                        <div className="flex items-center gap-3 mb-4 border-b border-white/10 pb-3">
                          <Avatar id={p.avatarId} size={48} frameThemeId={prof?.equipped?.frame} />
                          <div>
                        ...
                        <div className="flex justify-center gap-2">
                          {[...(myCustomEmojis.length > 0 ? myCustomEmojis.slice(0, 2) : []), "😂", "😡", "😢"].slice(0, 5).map((emoji) => (
                            <button
                              key={emoji}
