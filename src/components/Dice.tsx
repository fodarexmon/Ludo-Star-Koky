...
export const Dice = memo(function Dice({ value, rolling, size = 80, themeId }: { value: number | null; rolling?: boolean; size?: number; themeId?: string }) {
  ...
  const activeTheme = STORE_ITEMS.find((i) => i.id === themeId)?.diceTheme || STORE_ITEMS.find((i) => i.id === "dice_default")!.diceTheme!;

  const renderFace = (faceValue: number, transform: string) => (
    <div
      className="absolute inset-0 rounded-2xl"
      style={{
        background: activeTheme.face,
        transform,
    ...
    >
      {DOTS[faceValue]?.map(([x, y], i) => (
        <span
          ...
          style={{ background: activeTheme.dot, left: `${x * 100}%`, top: `${y * 100}%`, width: size * 0.18, height: size * 0.18, transform: "translate(-50%, -50%)" }}
        />
      ))}
    </div>
  );
